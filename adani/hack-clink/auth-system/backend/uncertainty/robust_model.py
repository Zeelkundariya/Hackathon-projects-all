"""Robust (worst-case) optimization model.

Business meaning:
- We want a single production & transport plan that is feasible for ALL scenarios.
- Instead of minimizing expected cost, we minimize the cost under the worst inventory outcome.

Why this helps:
- It produces "safer" plans (more buffer) that remain feasible even if demand is high.
- It is typically more conservative (higher cost) than expected-cost optimization.

Model:
- Same constraints as the stochastic model (feasible in every scenario).
- Objective: production + transport + worst-case holding cost.

Note:
- Production/transport costs are shared and counted once.
- Holding cost is scenario-dependent (because inventories differ by scenario).
- We introduce a variable Z that upper-bounds the holding cost in each scenario.
  Minimizing Z forces the plan to reduce the worst scenario's inventory carrying cost.
"""
...
def build_robust_model(data: OptimizationData, scen: ScenarioDemandData) -> pyo.ConcreteModel:
    m = pyo.ConcreteModel()
    ...
    # Robust objective helper
    m.WorstHoldCost = pyo.Var(within=pyo.NonNegativeReals)
    ...
    def worst_hold_cost_bound(mm, s):
        scen_hold = sum(mm.HoldCost[p] * mm.Inv[s, p, t] for p in mm.P for t in mm.T)
        return mm.WorstHoldCost >= scen_hold

    m.WorstHoldBound = pyo.Constraint(m.S, rule=worst_hold_cost_bound)
    ...
    def robust_obj(mm):
        ...
        return prod_cost + trans_cost + mm.WorstHoldCost

    m.Objective = pyo.Objective(rule=robust_obj, sense=pyo.minimize)
